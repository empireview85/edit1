/* global Chart */
// Surfaces any otherwise-silent JS error as a visible popup instead of the screen just doing
// nothing — makes it possible to diagnose a broken deploy without access to the browser console.
...
// separate, throwaway path in the SAME Firebase database instead of the real one — same login,
// same Firebase project, zero risk to live guest/money data. Logging in still works normally
// (accounts live at 'users'/'bannedUsers', not under this path) but every room/guest you add or
// change only exists at DATA_ROOT, safe to experiment on (e.g. testing two tabs checking the same
// room out at once) before trusting a change against the real hotel data. Remove ?staging=1 (or
// just reload the normal URL) to go back to the live 'hotelData' node.
const DATA_ROOT = new URLSearchParams(location.search).get('staging') ? 'hotelData_staging' : 'hotelData';
...
const translations = {
    en: {
        ...
        exchange_rate_hint:'Used to convert between USD and IQD everywhere in the app (check-in, check-out, deposits, reports).',
    ...
    ar: {
        ...
        exchange_rate_hint:'يُستخدم لتحويل العملة بين الدولار والدينار العراقي في كل مكان بالتطبيق (تسجيل الدخول، تسجيل الخروج، العربون، التقارير).',
...
// A single exchange rate, used both directions — this app used to offer separate buy/sell rates,
// but that was simplified back down to one shared rate at the user's request. Falls back to the
// old single `exchangeRate` setting (then 1500) for hotels that haven't set rateUSDtoIQD yet.
...
function migrateRoomStatuses(rooms, rawStatuses) {
    ...
    return (rooms || []).map(r => {
        ...
        // A room can't legitimately be both "available to book" and have a guest currently in
        // it — this combination has shown up in live data (most likely a sync race between two
        // devices/tabs each writing their own stale copy of the same room) and renders as a
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const sidebarVerEl = document.getElementById('sidebarVersionTag');
    if (sidebarVerEl) sidebarVerEl.textContent = `Management System ${APP_VERSION}`;
    // Init language
        ...
        window.fbAuth.onAuthStateChanged(user => {
            if (user) {
                window.fbDb.ref('users/' + user.uid).once('value').then(snap => {
                    ...
                    return window.fbDb.ref(DATA_ROOT).once('value').then(dataSnap => {
                        const fbData = dataSnap.val();
                        if (fbData) { hotelData = fbMerge(fbData); localStorage.setItem(DATA_ROOT, JSON.stringify(hotelData)); captureSyncSnapshot(); }
                        showApp();
...
function displayCheckInRooms(rooms) {
    ...
    rooms.forEach(room => {
        ...
        // currentGuest is the source of truth for "someone is actually in this room" — not the
        // status label, which can drift out of sync (e.g. a stale sync from another device/tab).
        const isOccupied = !!room.currentGuest;
...
function handleCheckIn(e) {
    ...
    const roomId = parseInt(document.getElementById('roomSelect').value);
    const room = hotelData.rooms.find(r => r.id === roomId);
    if (!room) { showToast(t('toast_room_unavailable'), 'error'); return; }

    const roomCfg = getStatusConfig(room.status, room);
    if (!roomCfg.bookable && room.status !== 'reserved') {
...
function loadCheckOutForm(roomId) {
    ...
    const guest = hotelData.guests.find(g => g.id === room.currentGuest.id);
    if (!guest) {
        // room.currentGuest points at a guest record that no longer exists (leftover corrupted
        // data from the sync bug — see saveDataToStorage). There's no guest data to protect here,
        // so this just clears the dangling pointer — nothing to check out, nothing to lose.
        const content = document.getElementById('checkOutContent');
        content.innerHTML = `
            <div style="text-align:center;padding:40px 24px;">
                <div style="width:70px;height:70px;border-radius:50%;background:#fef2f2;display:flex;align-items:center;justify-content:center;margin:0 auto 18px;">
                    <i class="fas fa-triangle-exclamation" style="font-size:2rem;color:#dc2626;"></i>
                </div>
                <h3 style="font-size:1.2rem;font-weight:700;color:#1f2937;margin-bottom:8px;">Room ${room.number} — Broken Data</h3>
                <p style="color:#6b7280;font-size:0.92rem;max-width:420px;margin:0 auto 22px;">
                    This room shows a guest, but that guest's record no longer exists — there's nothing to check out.
                    ${loggedInUser?.role === 'admin' ? 'Click below to free up the room.' : 'Ask an admin to fix this room.'}
                </p>
                ${loggedInUser?.role === 'admin' ? `
                <button onclick="adminClearBrokenRoom(${room.id})" class="btn btn-secondary" style="justify-content:center;">
                    <i class="fas fa-unlock"></i> Free Up Room ${room.number}
                </button>` : ''}
            </div>`;
        return;
    }
        ...
        <div class="mb-5">
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                ...
                <div class="info-item" style="border-left-color: #f59e0b; grid-column: 1 / -1;">
                    ...
                    <div class="text-xs text-gray-400 mt-1">${t('exchange_rate_note')||'Rate used'}: $1 = IQD ${fmtIQD(getRateUSDtoIQD())}</div>
                </div>
            </div>
        </div>
...
// Fixes a room stuck showing an occupied guest whose checkout was already recorded (a sync bug —
// see saveDataToStorage). Only clears the room's currentGuest/status; never touches the guest's
// checkout fields, payment amounts, or checkedOutAt date, so it can never affect revenue totals —
// unlike re-running the normal Check Out flow, which would recollect payment and overwrite the
// original checkout date, making the same stay count twice across different days' shift reports.
function adminReleaseDesyncedRoom(roomId) {
    if (loggedInUser?.role !== 'admin') return;
    if (!requireOnline()) return;
    const room = hotelData.rooms.find(r => r.id === roomId);
    if (!room || !room.currentGuest) { showToast('This room is no longer occupied — reload the page.', 'error'); return; }
    const guest = hotelData.guests.find(g => g.id === room.currentGuest.id);
    if (!guest || !guest.checkedOutAt) {
        showToast('This guest is not recorded as checked out — use the normal Check Out flow so payment gets collected.', 'error');
        return;
    }
    if (!confirm(`Room ${room.number} still shows ${guest.name} as occupying it, even though their checkout on ${new Date(guest.checkedOutAt).toLocaleString()} was already recorded. Release the room without collecting payment again?`)) return;

    room.currentGuest = null;
    room.status = 'checkout';
    addActivity(`Admin released Room ${room.number} — ${guest.name}'s checkout (already recorded ${new Date(guest.checkedOutAt).toLocaleString()}) hadn't cleared the room; no new payment collected.`);
    saveDataToStorage();
    showToast(`Room ${room.number} released.`, 'success');
...
// Clears a room whose currentGuest points at a guest record that no longer exists at all (not
// just an already-checked-out one — see adminReleaseDesyncedRoom for that case). There's no guest
// data behind the pointer, so nothing to preserve or bill; this just frees the room.
function adminClearBrokenRoom(roomId) {
    if (loggedInUser?.role !== 'admin') return;
    if (!requireOnline()) return;
    const room = hotelData.rooms.find(r => r.id === roomId);
    if (!room || !room.currentGuest) { showToast('This room is no longer occupied — reload the page.', 'error'); return; }
    const stillExists = hotelData.guests.find(g => g.id === room.currentGuest.id);
    if (stillExists) { showToast('This guest record exists after all — reload and try again.', 'error'); return; }
    if (!confirm(`Room ${room.number} points at a guest record that no longer exists. Free up the room? Nothing is billed.`)) return;

    const staleName = room.currentGuest.name || '—';
    room.currentGuest = null;
    room.status = 'checkout';
    addActivity(`Admin freed Room ${room.number} — its linked guest record ("${staleName}") no longer existed.`);
    saveDataToStorage();
    showToast(`Room ${room.number} freed up.`, 'success');
    loadCheckOutPage();
}
...
function openChangeRoomModal(roomId) {
    ...
    // Pre-fill with whatever price is currently on record, so leaving the fields untouched keeps
    // the guest's existing agreed price — pick a new room from the dropdown to overwrite it with
    // that room's listed price instead.
    const priceIQDEl = document.getElementById('changeRoomPriceIQD');
    const priceUSDEl = document.getElementById('changeRoomPriceUSD');
    if (isGuestMove) {
        const guest = hotelData.guests.find(g => g.id === room.currentGuest.id);
        if (priceIQDEl) priceIQDEl.value = guest?.basePriceIQD ? Math.round(guest.basePriceIQD).toLocaleString('en-US') : '';
        if (priceUSDEl) priceUSDEl.value = guest?.basePriceUSD || '';
    } else {
        if (priceIQDEl) priceIQDEl.value = room.reservationInfo.priceIQD ? Math.round(room.reservationInfo.priceIQD).toLocaleString('en-US') : '';
        if (priceUSDEl) priceUSDEl.value = room.reservationInfo.priceUSD || '';
    }

    openModal('changeRoomModal');
}

// Selecting a candidate room suggests that room's listed price (matching how Check-In pre-fills
// price when a room is picked) — still freely editable afterward for a custom agreed price.
function handleChangeRoomSelection() {
    ...
    const priceUSDEl = document.getElementById('changeRoomPriceUSD');
    if (priceIQDEl) priceIQDEl.value = '';
    if (priceUSDEl) priceUSDEl.value = newRoom.price || '';
}
...
function confirmChangeRoom() {
    ...
    const enteredPriceIQD = parseFloat((document.getElementById('changeRoomPriceIQD')?.value || '').replace(/,/g, '')) || 0;
    const enteredPriceUSD = parseFloat(document.getElementById('changeRoomPriceUSD')?.value) || 0;
    ...
    if (_changingRoomMode === 'guest') {
        ...
        // Fall back to whatever the guest already had if both price fields were left empty,
        // rather than silently zeroing out an existing agreed price.
        const priceChanged = enteredPriceIQD > 0 || enteredPriceUSD > 0;
        const oldPriceIQD = guest.basePriceIQD || 0, oldPriceUSD = guest.basePriceUSD || 0;
        guest.basePriceIQD = priceChanged ? enteredPriceIQD : oldPriceIQD;
        guest.basePriceUSD = priceChanged ? enteredPriceUSD : oldPriceUSD;
        guest.basePrice    = guest.basePriceIQD || guest.basePriceUSD;
        ...
        const priceNote = priceChanged
            ? ` (price updated to ${guest.basePriceIQD ? `IQD ${fmtIQD(guest.basePriceIQD)}` : `$${guest.basePriceUSD}`}/night)`
            : '';
        addActivity(`Guest ${guest.name} moved from Room ${oldRoom.number} to Room ${newRoom.number}${priceNote}`);
    ...
    const resPriceChanged = enteredPriceIQD > 0 || enteredPriceUSD > 0;
    if (resPriceChanged) {
        ri.priceIQD = enteredPriceIQD;
        ri.priceUSD = enteredPriceUSD;
        ri.price = enteredPriceUSD || enteredPriceIQD || newRoom.price;
    }
...
function removeUser(index) {
    ...
    if (user.uid && window.fbDb) {
        ...
        window.fbDb.ref(`${DATA_ROOT}/users`).once('value').then(snap => {
            const users = snap.val() || [];
            const filtered = Array.isArray(users)
                ? users.filter((_, i) => i !== index)
                : Object.values(users).filter(u => u.uid !== user.uid);
            window.fbDb.ref(`${DATA_ROOT}/users`).set(filtered).catch(() => {});
        }).catch(() => {});
    }
...
function showLogin() {
    ...
    if (overlay) {
        ...
        const verEl = document.getElementById('loginVersionTag');
...
function handleLogin(e) {
    ...
    window.fbAuth.signInWithEmailAndPassword(email, password)
        .then(cred => {
            ...
            // Everything past this (loading hotelData, assigning loggedInUser, showApp(), attaching the
            // realtime listener) is deliberately NOT duplicated here — the onAuthStateChanged handler
            // fires for this exact sign-in too, and used to run that same sequence a second time in
            // parallel with this one. Two independent fetches racing to be "the" fresh copy, and two
            // realtime listeners both attached to hotelData, was a real source of unpredictable stale-
            // looking screens after switching accounts in the same tab. Sign in here, let that one
            // handler be the single place that actually loads data and shows the app.
...
// Snapshot of every room/guest exactly as they looked the last time this tab loaded fresh data
// from Firebase (login, or a realtime update) — used to tell which records THIS tab actually
...
// keys (e.g. after a round-trip through Firebase, which doesn't preserve key order) must compare
// equal, or a harmless reordering would look like a real edit and trigger an unnecessary re-stamp.
...
// Records the current state of every room/guest as the new "last known good from Firebase"
...
// have been sitting idle/backgrounded for a while) marks a different room clean, for instance.
// A plain .set(hotelData) replaces the ENTIRE database with whatever this one tab currently has
// in memory — so a slightly-stale tab can silently erase a room/guest that changed elsewhere.
//
// Fix: stamp a private _rev (timestamp) on ONLY the records this tab actually changed since its
// last known-good snapshot, then merge each record against a fresh server read by _rev — whichever
// side is newer wins. A record this tab never touched keeps its old _rev, so it always loses to a
// genuinely fresher edit from another device instead of clobbering it.
function saveDataToStorage() {
    if (!(window.fbDb && loggedInUser)) return;

    // Persist locally immediately, before any network round-trip, so the change survives even if
    ...
    const stampChanged = (arr, snapshotKey) => {
        (arr || []).forEach(record => {
            ...
            const baseline = _syncSnapshot[snapshotKey][record.id];
            if (baseline === undefined || stableStringify(rest) !== baseline) record._rev = now;
        });
    };
    stampChanged(hotelData.rooms,  'rooms');
    ...
    window.fbDb.ref(DATA_ROOT).once('value').then(snap => {
        ...
        const mergeByRev = (localArr, serverArr, tombstoneList) => {
            ...
            const result = [];
            new Set([...localById.keys(), ...serverById.keys()]).forEach(id => {
                if (deletedIds.has(id)) return;
                const mine = localById.get(id), theirs = serverById.get(id);
                if (mine && !theirs) { result.push(mine); return; }
                if (!mine && theirs) { result.push(theirs); return; }
                result.push((theirs._rev || 0) > (mine._rev || 0) ? theirs : mine);
            });
            return result;
        };
        dataToSave.rooms  = mergeByRev(dataToSave.rooms,  toArr(server.rooms),  dataToSave.deletedRoomIds);
        dataToSave.guests = mergeByRev(dataToSave.guests, toArr(server.guests), dataToSave.deletedGuestIds);
        window.fbDb.ref(DATA_ROOT).set(dataToSave).then(finish).catch(onFail);
    }).catch(() => {
        // Offline or the read failed — still save what we have rather than losing the change.
        window.fbDb.ref(DATA_ROOT).set(dataToSave).then(finish).catch(onFail);
    });
}
...
function loadDataFromStorage() {
    try {
        const stored = localStorage.getItem(DATA_ROOT);
        if (stored) {
...
function viewRoomDetails(roomId) {
    ...
    if (room.currentGuest) {
        ...
        if (guest) {
            ...
            // The guest's own record already shows they checked out (payment already recorded) —
            // this room just never got the memo (a sync bug, now fixed going forward, but rooms
            // already stuck this way stay stuck until manually released). Re-running the normal
            // Check Out flow here would recollect payment and overwrite the original checkout date,
            // which is how the same money ends up counted in two different days' shift reports —
            // so give admins a way to clear just the room→guest link, with no payment involved.
            if (guest.checkedOutAt && loggedInUser?.role === 'admin') {
                guestHtml += `
                    <div style="background:#fffbeb;border-left:4px solid #f59e0b;border-radius:8px;padding:14px;margin-top:10px;">
                        <p style="color:#92400e;font-size:0.85rem;margin-bottom:8px;">
                            <i class="fas fa-triangle-exclamation" style="margin-right:6px;"></i>
                            ${guest.name}'s checkout was already recorded on ${new Date(guest.checkedOutAt).toLocaleString(locale)}, but this room still shows them as occupying it (a sync bug). Do not check out again — that would recollect payment and re-date the checkout, causing it to be counted twice across shift reports.
                        </p>
                        <button onclick="adminReleaseDesyncedRoom(${room.id})" class="btn btn-secondary" style="width:100%;">
                            <i class="fas fa-unlock"></i> Release Room (no payment — already recorded)
                        </button>
                    </div>`;
            }
        }
...
function modifyCheckoutDate(roomId, newDateString) {
    ...
    if (!room || !room.currentGuest) return;
    const guest = hotelData.guests.find(g => g.id === room.currentGuest.id);
    if (!guest) { showToast('This room\'s guest record is missing — reload the page.', 'error'); return; }
...
function _populateOrderModal() {
    const infoEl = document.getElementById('orderRoomInfo');
    if (infoEl) {
        infoEl.textContent = selectedRoom
            ? `${t('room_prefix')} ${selectedRoom.number}${selectedRoom.currentGuest ? ' — ' + selectedRoom.currentGuest.name : ''}`
            : '';
    }
...
// Applies a freshly-fetched hotelData snapshot and refreshes whatever's currently on screen.
// Shared by the realtime listener and the visibility-change resync below — wrapped in try/catch
// so a bug in one page's refresh (e.g. a stale/renamed function) can never abort the sync itself
// and leave `hotelData` half-applied for every other page.
function applyFreshHotelData(data) {
    if (!data) return;
    try {
        ...
        console.error('applyFreshHotelData failed:', e);
    }
}

function setupFirebaseRealtimeListener() {
    // Guard against attaching a second .on('value') callback on top of one that's already live —
    // logout (.off()) always clears prior ones, but this makes the function safe to call more than
    // once within a single session too, instead of relying on every call site never doing that.
    ...
    // above doesn't reliably fire again the instant it comes back, so the screen can keep showing
    // whatever was true when it went to sleep even though other devices have since checked guests
    // out, moved rooms, etc. Forcing one fresh fetch the moment the app becomes visible again
    // closes that gap regardless of whether the socket recovered.
    if (!window._visibilityResyncBound) {
        window._visibilityResyncBound = true;
        document.addEventListener('visibilitychange', () => {
            if (document.visibilityState === 'visible' && loggedInUser && window.fbDb) {
                window.fbDb.ref(DATA_ROOT).once('value')
                    .then(snap => applyFreshHotelData(snap.val()))
                    .catch(() => {});
            }
        });
    }
...
function refreshPage(pageId) {
    switch (pageId) {
        ...
        case 'rooms':         displayRooms(hotelData.rooms); break;
    ...
    window.fbAuth.createUserWithEmailAndPassword(email, password)
        .then(cred => {
            ...
            return window.fbDb.ref('users/' + uid).set(profile)
                .then(() => window.fbDb.ref(DATA_ROOT).set(hotelData));
        })
        .then(() => {
            if (btn) btn.disabled = false;
